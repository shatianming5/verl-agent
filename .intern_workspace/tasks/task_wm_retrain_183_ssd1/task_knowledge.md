...
44. **✅ 配置搜索完成(串行不满占, Session30; 短测法=tb2+VAL_BEFORE_TRAIN=False+PYTHONUNBUFFERED=1)**：三大结论——(a) **param_offload 对峰值+速度全无影响**(P0b off=T vs P1b off=F 同条件:update_actor 252.6 vs 253.5s、max_alloc 25.4 vs 24.3GB,差<1%)——坐实条目34,FSDP 反向须 all-gather 回参数,offload 只省闲置时段。full 不必纠结 param_offload。(b) **提速唯一有效杠杆=micro_batch**(纯显存/速度旋钮,不改结果:ppo_mini=256 不变,mb 只是梯度累积分块,数学等价):mb4 update_actor 253s/nvsmi 28GB; **mb8 145s(快1.75x)/nvsmi 38GB**; mb16 nvsmi 48G→**OOM 即便独占 49G 卡**(FSDP 33.7G+vLLM 保留+ctx 超单卡,出局)。(c) **gen(rollout)是下一地板**(1412s/步 占33%,不受 mb 影响,受 enforce_eager 逐token 限;CUDA graph 提速被 free_cache_engine=True 挡)。full 推算:mb4=71min/步·7.4天/run·峰28G(安全+jusheng ok);**mb8≈51min/步·≈5.3天/run·峰38G(省~2天/run,需独占;+jusheng=49G 顶格 OOM 险,SAVE_FREQ=15 使崩溃≤15步可 resume)**。⚠️脚本默认 PPO_MICRO=16 会 OOM,full 必须覆盖 PPO_MICRO=8(推荐/独占)或 4(安全/可共卡)。测量工具:files/srch_launch.sh(短测启动器)+srch_watch.sh(抓 verl step:1 metrics)+srch_kill.sh(清理),已复用。
