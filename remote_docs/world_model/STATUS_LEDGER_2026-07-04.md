...
辅助损失 = teacher-forced 的 next-obs token 交叉熵,权重 `λ_obs`。已核实**未**被任何 latent 项污染(A 线日志中 latent loss = 0;证据见 `PROVENANCE.md`)。**以下均为 2-seed(seed0/1)聚合,非单 seed。**

| λ_obs | eval10x（2-seed 聚合） | 单 seed 明细 | 相对基线 Δ | 备注 |
|---|---|---|---|---|
| 0.001 | **0.5470 ± 0.0665** | s0=0.4869 / s1=0.6071 | −0.160 | |
| 0.01 | **0.6714** | s0=**0.7311** / s1=0.6118 | −0.035 | s0 已略超基线聚合 |
| 0.05 | 早期杀掉（不稳定） | s1 online val 0.18 | — | λ 过大破坏训练 |

**结论（更正，此前"越大越差单调"的说法有误）：** 关系**非单调,呈倒 U**——λ=0.01(0.671)反而比 λ=0.001(0.547)**更好**(越大越近基线),λ=0.05 过大而崩溃;0.01 是当前最优点。**λ=0.01 的 seed0=0.7311 甚至略超基线聚合 0.7065**,但 seed1=0.6118 把 2-seed 聚合拖到 0.671,**seed 方差很大、结论不稳**。三个条件的 2-seed 聚合仍都 < 基线。这与 B 线发现一致——成功轨迹对应*更高*的 obs-CE(novelty),把 CE 往下压方向可疑;但 λ=0.01 尚未被排除、值得补 seed 降方差。
